...
for state_folder in os.listdir(census_dir):
    ...
    if os.path.exists(age_sex_file):
        ...
        for _, row in data.iterrows():
            ...
            output[state_folder][county_city]["age_sex"] = process_csv(
                pd.DataFrame([row]), age_sex_pattern, "age_sex", total_population
            )
    ...
    if os.path.exists(household_file):
        ...
        for _, row in data.iterrows():
            ...
            output[state_folder][county_city]["household_type"] = process_csv(
                pd.DataFrame([row]), household_pattern, "household_type", total_population
            )

    # Add "Total" entry for the state as the first item
    output[state_folder] = OrderedDict([("Total", {
        "age_sex": output[state_folder]["Total"]["age_sex"] if "Total" in output[state_folder] else {},
        "household_type": output[state_folder]["Total"]["household_type"] if "Total" in output[state_folder] else {}
