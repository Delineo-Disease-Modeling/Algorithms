...
- **Stage 4 — robust generic-visitor dwell.** Generic arrivals now draw from the five SafeGraph dwell buckets rather than `median_dwell ± 1 hour`. Each POI distribution is shrunk toward same-month, same-state NAICS peers with weight `unique_visitors / (unique_visitors + 20)`; exact six-digit NAICS backs off to four-digit, sector, then state-wide when fewer than 30 trusted peers remain. A POI falls back completely only when at least two of median dwell, `>240` share, and full bucket shape lie beyond the category's empirical 1st/99th-percentile envelope, or when corroborating shared/enclosed-child, near-home repeat-device, stale-closure, or night-plus-weekend evidence is present. The open-ended `>240` bin uses a decreasing category-calibrated tail. Scheduled workers and students remain controlled by their explicit schedule blocks.
...
- **S1 (Algorithms) — DONE** (`ryad/movement-redesign`, commit `5458cd3`). `popgen.convert_data` emits `catchment_fj` per POI into the places bundle, reusing the shared `_catchment_fraction` + `_median_fj_fallback` (so movement‑target and bundle `f_j` can't drift), floored at `0.02`. *Verified on real Barnsdall: every POI gets `f_j ∈ [0.02,1]`; 22.8% missing → median fallback; visit‑weighted mean `f_j = 0.151`, reproducing §8. Off‑path unchanged.*
- **S2 (Simulation) — DONE** (`ryad/external-foi`, commits `f2828b3`, `e209690`). `world.parse_facility` parses + validates `catchment_fj`; `runner` precomputes `store.ext_ratio = (1−f_j)/f_j × emit_factor` per location (0 for households / unknown‑`f_j`); `_vectorized_transmission` adds `W_external = n_internal × ext_ratio × P_ext` to each room's emission, and no longer early‑returns on `infectious.any()` when the term is active (so it can cold‑start‑seed). Flag `external_foi` **default off**; `external_prevalence` (P_ext) default **0** (inert even when flagged on); `external_emit_factor` default 1. *Verified: whole 94‑test suite passes unchanged (golden preserved); end‑to‑end on real Barnsdall (czone‑107) the term alone seeds an outbreak from zero internal seed, is inert at P_ext=0, and gives households no term; 9 fixture‑free regression tests.* **Scope/limitation:** implemented in the SoA engine kernel only (the default‑on, every‑timestep path); the non‑engine pairwise/aggregate paths don't carry it yet (those are infection‑gated, so they couldn't cold‑start‑seed anyway) — a run that requests the term but falls back off‑engine logs a warning. Non‑engine coverage = follow‑up.
- **S3** — wire a richer `P_ext(t)` (today a config scalar `external_prevalence`; next, a time series / county‑case curve indexed by sim time — the kernel already keeps `P_ext` as a runtime scalar to make this a drop‑in).
- **S4** — validate the cross‑repo wiring end‑to‑end (Algorithms emits `catchment_fj` → Simulation consumes it on a freshly generated zone, not just the injected fixture), run the closed‑vs‑background‑FOI ablation (§6 "decisive experiment" — does the term move the outbreak threshold?), then **calibrate** `external_prevalence` + `movement_scale` together against the validation harness.
